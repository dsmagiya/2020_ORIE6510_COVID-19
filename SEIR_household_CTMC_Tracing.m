...
% modified 5/7/2020 Desheng Ma dm852: add tracing (test rate)

%clear all 

function [sBenchmark,iBenchmark,ieBenchmark,eeBenchmark,...
    rBenchmark,dBenchmark, ...
    iiiBenchmark]=SEIR_household_CTMC_Tracing(testrate)
...
% set parameters
radius = 2; % affected radius of an infected person, who are isloated along with him/her
b = 0.08; % probability of transmission, 95% CI [0.05068, 0.05429]
k = [30 30 10]*(1-testrate); % k = average # people exposed to by infected ppl
beta = k.*b; % infection rate, parameterize beta=k*b
incubation = 8;
delta = [1/(100*incubation) 1/incubation 1/incubation]; % exposed -> infected rate, i.e. 1/incubation period
rec = 21; %three weeks to recover
gam = [1/rec 1/rec 1/rec]; % recover rate, 1/recovery period
alpha = [0.00 0.10 0.15]/5; % death rate
...
iii_ri = {0 0 0}; % cumulative reported infected 
...
NN = N -radius*(ri{1}(j)+ri{2}(j)+ri{3}(j))-(d{1}(j)+d{2}(j)+d{3}(j)); % deaths are removed from the population, sadly
...
%            iiiriBenchmark(kk,:)=[iii_ri{1}(j+1),iii_ri{2}(j+1),iii_ri{3}(j+1)];
           end
...
%     iiiriBenchmark(n,iiiriBenchmark(n,:)==0)=iiiriBenchmark(n-1,iiiriBenchmark(n,:)==0);
