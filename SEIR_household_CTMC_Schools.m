...
function [sBenchmark,iBenchmark,ieBenchmark,eeBenchmark,rBenchmark,dBenchmark, iiiBenchmark]=SEIR_household_CTMC_Schools(kids_e_i)

TN=30000; % NYS total population is about 20*10^6, NYC total population is 8*10^6
eP=0.164; % elderly proportion
yP=0.40; % estimated youth/student proportion(<25) Cnesus gives up to <18 (~20%), but here would like to include university students
...
% %---- test
% % set parameters
% beta = [kids_e_i 1 1]; % infection rate
% delta = [0.5 0.5 0.5]; % exposed -> infected rate, i.e. 1/incubation period
% gam = [0.1 0.1 0.1]; % recover rate
% alpha = [0.01 0.05 0.07]/7; % death rate
% %----
