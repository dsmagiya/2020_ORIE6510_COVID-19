...
for n=1:simRound
    [s,i,ie,ee,r,d,iii]=SEIR_household_CTMC_Tracing(testrate(seir));
    sCum(n,:,:)=s;
    iCum(n,:,:)=i;
    ieCum(n,:,:)=ie;
    eeCum(n,:,:)=ee;
...
stitle = sprintf('SEIR-household-CTMC-Tracing: Testrate = %g', testrate(seir));
title(stitle)
grid on;
